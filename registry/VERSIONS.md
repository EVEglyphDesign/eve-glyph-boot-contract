...
| v1.7 | L1.7 | 2026-09-22 | **Paired observations for the silvatrading.com incident of 2026-09-21.** Register-only: appended ROO-2026-09-21-03 (Perplexity claimed an exhaustive repository search without stating its bounded scope), ROO-2026-09-21-04 (ninety minutes of cross-harness signals routed through the operator's chat instead of the shared git remote), and ROO-2026-09-21-05 (Perplexity drafted an observations file for this incident without first reading Claude's sibling file filed forty minutes earlier). No change to `README.md`, `skill/SKILL.md`, or any docs surface — this row records observations that landed as siblings to the Claude-authored account, per the newly added rule 7 in `EVEglyphDesign/eve-external-surfaces/registry/silvatrading.com-incident-2026-09-21.md`. Deploy of silvatrading.com to the correct serving repo (`EVEglyphDesign/silvatrading-com` commit `5be0b82`, tag `deploy-2026-09-21-enterprise-system-services`) closed in the same session. | `git revert` the commit that lands this row, or `git checkout v1.6-active-md -- registry/OBSERVATIONS.md registry/VERSIONS.md` | `v1.7-observations-paired` | n/a — reversible |
| v1.8 | L1.8 | 2026-09-25 | **EgD-BOOT-008 — Writers and readers.** Added §12 to `README.md` and §4f to `skill/SKILL.md`: three roles (Writer, Linker, Reader), Reader as the default for every delegated task, one writer per path, breach under existing class **D** — no new class. First Reader in service: [`sapfans-io/scripts/check-links.py`](https://github.com/EVEglyphDesign/sapfans-io/blob/main/scripts/check-links.py) (`sapfans-io` tag `v2.1-link-check`). Pattern credit: [undefined-ui/second-brain-os](https://github.com/undefined-ui/second-brain-os) (MIT), idea only. | `git revert` the commit that lands this row, or `git checkout v1.7-observations-paired -- README.md skill/SKILL.md registry/VERSIONS.md` | `v1.8-boot-008-roles` | n/a — reversible |
