#!/usr/bin/env python3
"""Render EgD-STR-002.md into the public /strategy/ page. Single source of truth."""
import re, hashlib
...
def body(md):
    ...
    while i < len(lines):
        ...
        if ln.startswith("|") and i + 1 < len(lines) and re.match(r"^\\|[\\s:\\-|]+\\|$", lines[i+1].strip()):
            fp(); fl()
            cells = lambda x: [c.strip() for c in x.strip().strip("|").split("|")]
            hdr = cells(ln); i += 2; rows = []
            while i < len(lines) and lines[i].strip().startswith("|"):
                rows.append(cells(lines[i])); i += 1
            out.append("<table><thead><tr>" + "".join(f"<th>{inline(c)}</th>" for c in hdr)
                       + "</tr></thead><tbody>"
...
<title>Hook, Context, Rehook, Payoff \u2014 EgD-STR-002 \u00b7 EVEglyphDesign</title>
...
<section class="hero">
  ...
  <p class="meta">
    <b>Document ID</b> EgD-STR-002 &nbsp;\u00b7&nbsp; <b>Key ID</b> EgD-KEY-2026-07 &nbsp;\u00b7&nbsp; <b>Status</b> strategy note, v1.0<br>
  ...
  <div class="cta">
    ...
    <a class="btn ghost" href="https://open.spotify.com/episode/1JGZOWkjNkNeDwS24ah9md">Listen to the source episode</a>
  </div>
</section>
<main>
%(body)s
