...
description: "Load FIRST, before any other work, on every EVEglyphDesign, EgD, EVE Glyph, PAIX, URIEL, canon, parish, or sovereign-data request. The binding processing and output contract EgD-BOOT-001: the six-rung cheapest-source-first retrieval ladder, the three-thread recall rule, the free/cheap/expensive spend classes and where the interrupt belongs, the Burn Ledger measurement gate (EgD-BOOT-002), model routing and the EgD-BURN switch for live client work (EgD-BOOT-007), repository-only record keeping (EgD-BOOT-004), versioning and reversibility (EgD-BOOT-005), the rule of three (EgD-BOOT-006), and the output canon (PDF by default, clickable links only, cream-and-orange palette, Fraunces and Inter, EVEglyphDesign naming, land in the repo and on a public surface). Also load when asked about token spend, credit burn, burn rate, which model to run, economy or burn lanes, usage analytics, wasted processing, cold starts, defect logging, or the observation and defect registers."
...
| `registry/OBSERVATIONS.md` | The defect register — see §5 |
...
- **New material is added only after the operator has consciously chosen it** — added 2026-09-10. Before anything new lands (canon rule, charter article, reference, option, library, page, section, component, copy paragraph, file, or link the operator did not name), propose the addition in one line and wait. Fixing a defect the operator flagged is not an addition; propagating that fix into surfaces the operator has not yet named **is**. "Do it everywhere" authorises propagation of the specific fix, not further additions discovered along the way. Breach = **S**; recurring = **P**.
- **The default context is the operator's own history, not the outside world** — added 2026-09-10. Before any task begins, read the operator's record first: session context, memory, the knowledge wiki, the repository, prior sessions, prior corrections, published reference set. That record is the frame every answer is built inside. External sources are consulted only after the record has been read, and only for what the record does not contain. Where the operator's history and the outside world disagree, history wins without asking. Breach ladder: **R** if the record already answered, **P** if an external source overrode the record without asking, **B** if the record was never read at all.
- **Apply the operator's reference set, do not generate design** — added 2026-09-10. Where a reference exists (template, prior surface, committed component, wiki decision), use it verbatim: class names, tokens, geometry, copy patterns. Where the reference set is silent, ask the operator once and wait — do not fill silence with a novel palette, layout, component, theme system, animation, or interaction pattern. The rule is not "generate carefully"; it is "do not generate." Breach = **P**; displaces an existing reference = **C**.
- **Security gates default to the conservative choice from operator history** — added 2026-09-10. Compliance spine, security lane, adoption gate, lead-tier band, custody notice, provenance stamp, permission pill: resolved against the operator's own record first. External search, design-trend inspiration, framework defaults, fresh generation are not eligible inputs. Where operator history is silent, the conservative choice is the one that fails safely — fixed colours over inheritable tokens, static assets over runtime rendering, published precedent over novel pattern. Breach = **C**; recurring = **P**.
- **No mid-task internet scanning for novel material the operator did not request** — added 2026-09-10. External search during a task is restricted to a specific fact the operator asked for or the task strictly requires. Whatever the model happens to notice stays out of the response. A new option, tool, article, framework, template, or reference surfaces to the operator only if the operator asked for options, or as a one-line note at the end of the finished task that the operator can ignore. Breach = **P**; recurring on the same task = **B**.
- **Themed HTML surfaces read back on both themes at phone width before called live** — added 2026-09-10. Permanent-ink surfaces (`.lane.security`, `.tier.lead-tier`, `.spine`, any equivalent) pin their own colour locally rather than inheriting a theme token that a theme block can invert. Breach = **C**; recurring on the same surface = **D**.
...
Append a row to `registry/OBSERVATIONS.md` in
